...
int B_PLUS_TREE_LEAF_PAGE_TYPE::Insert(const KeyType &key, const ValueType &value, const KeyComparator &comparator) {
  ...
  int index = left;
  memmove(static_cast<void *>(array + index + 1),
          static_cast<void *>(array + index),
...
bool B_PLUS_TREE_LEAF_PAGE_TYPE::Lookup(const KeyType &key, ValueType *value, const KeyComparator &comparator) const {
  ...
  while (left <= right)
  {
    ...
    else if(comparator(array[mid].first, key) > 0) right = mid - 1;
    else left = mid + 1;
  }
...
int B_PLUS_TREE_LEAF_PAGE_TYPE::RemoveAndDeleteRecord(const KeyType &key, const KeyComparator &comparator)
{
  ValueType value;
  int index;
  if (!Lookup(key, &value, comparator)) return GetSize();
  index = KeyIndex(key, comparator);
  memmove(static_cast<void *>(array + index),
          static_cast<void *>(array + index + 1),
